...
export const BidAdjustmentModal: React.FC<BidAdjustmentModalProps> = ({ campaign, onClose, onSave }) => {
  ...
  const getStrengthColor = (strength: BidStrength) => {
    switch (strength) {
      case 'Forte': return 'text-[color:var(--sl-fg-base)] bg-green-50 border-green-100';
      case 'Intermediário': return 'text-[color:var(--sl-fg-base)] bg-yellow-50 border-yellow-100';
      case 'Fraco': return 'text-[color:var(--sl-fg-base)] bg-red-50 border-red-100';
    }
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4 safe-top safe-bottom">
      ...
      <div className={modalClasses}>
        <div className="px-4 py-4 sm:px-8 sm:pt-6 sm:pb-5 border-b border-gray-100 flex items-center justify-between bg-white shrink-0">
          <div className="flex items-start gap-3 sm:gap-4 min-w-0">
            <div className="min-w-0">
              <h3 className="text-lg sm:text-xl font-semibold tracking-tight text-[color:var(--sl-fg-base)] truncate">Ajustar lances</h3>
              <p className="text-sm text-[color:var(--sl-fg-base-soft)] mt-0.5 sm:mt-1 font-normal truncate">{campaign.title}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 sm:gap-6 shrink-0">
             <div className="flex flex-col items-end hidden sm:flex gap-1">
                <span className="text-[11px] text-[color:var(--sl-fg-base-muted)] font-normal">Score Geral</span>
                <span className={`px-2.5 py-0.5 rounded-md text-xs font-medium border mt-1 ${getStrengthColor(overallScore as BidStrength)}`}>
                  {overallScore}
                </span>
             </div>
            <button type="button" onClick={handleClose} className="min-w-[44px] min-h-[44px] w-11 h-11 flex items-center justify-center rounded-xl hover:bg-gray-100 text-[color:var(--sl-fg-base-muted)] transition-colors touch-manipulation" aria-label="Fechar">
        ...
        <div className="px-4 sm:px-8 pb-1 pt-4 sm:pt-6 grid grid-cols-12 gap-4 sm:gap-8 items-center min-w-[480px] sm:min-w-0">
            <div className="col-span-5 text-[12px] font-normal text-[color:var(--sl-fg-base-muted)]">Mídia</div>
            <div className="col-span-3 text-[12px] font-normal text-[color:var(--sl-fg-base-muted)]">Lance atual</div>
            <div className="col-span-2 text-[12px] font-normal text-[color:var(--sl-fg-base-muted)]">Sugerido</div>
            <div className="col-span-2 text-[12px] font-normal text-[color:var(--sl-fg-base-muted)] text-right"></div>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-auto bg-white custom-scrollbar scroll-touch">
          <div className="flex flex-col px-4 sm:px-8 min-w-[480px] sm:min-w-0">
                ...
                return (
                  <div key={bid.mediaType} className="py-4 grid grid-cols-12 gap-4 sm:gap-8 items-center group border-b border-gray-100 last:border-0">
                    ...
                    <div className="col-span-5 flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl bg-gray-50 text-[color:var(--sl-fg-base-soft)] flex flex-shrink-0 items-center justify-center border border-gray-100 group-hover:border-gray-200 group-hover:bg-white transition-colors">
                        <span className="material-symbols-outlined text-[20px]">{getMediaIcon(bid.mediaType)}</span>
                      </div>
                      <div className="flex flex-col min-w-0">
                        <span className="text-sm font-medium text-[color:var(--sl-fg-base)] truncate" title={bid.mediaType}>{bid.mediaType}</span>
                        <div className="mt-1">
                    ...
                    <div className="col-span-3">
                        <div className="relative w-full">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[color:var(--sl-fg-base-muted)] text-sm pointer-events-none font-normal">R$</span>
                            <input
                                type="number"
                                step="0.01"
                                min="0"
                                ...
                                className="w-full pl-9 pr-12 min-h-[44px] h-11 bg-white border border-gray-200 rounded-lg text-base sm:text-sm font-normal text-[color:var(--sl-fg-base)] focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all placeholder-gray-400 tabular-nums"
                            />
                            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] font-normal text-[color:var(--sl-fg-base-muted)] pointer-events-none tracking-wide">
                              {pricingModel}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="col-span-2">
                        <span className="text-sm font-medium text-[color:var(--sl-fg-base-soft)] tabular-nums tracking-tight">
                    ...
                    <div className="col-span-2 flex justify-end">
                        {isDiff ? (
                            <button
                                ...
                                className="h-11 px-4 rounded-lg text-xs font-medium text-[color:var(--sl-fg-base-soft)] hover:text-[color:var(--sl-fg-base-soft)] hover:bg-blue-50 active:bg-blue-100 transition-all flex items-center justify-center"
                            >
                                Aplicar
                            </button>
                        ) : (
                           <div className="inline-flex items-center gap-1.5 px-2.5 py-1 h-8 rounded-full bg-green-50 border border-green-100">
                              <span className="material-symbols-outlined text-[14px] text-[color:var(--sl-fg-base-soft)] font-bold">check</span>
                              <span className="text-[11px] font-medium text-[color:var(--sl-fg-base)]">Aceito</span>
                           </div>
                        )}
                    </div>
            ...
            {bids.length === 0 && (
                <div className="text-center py-12 text-[color:var(--sl-fg-base-muted)] text-sm">
        ...
        <div className="px-4 sm:px-8 py-4 sm:py-5 border-t border-gray-100 bg-white flex flex-col-reverse sm:flex-row justify-end gap-3 safe-bottom">
          <button
            type="button"
            onClick={handleClose}
            className="min-h-[48px] px-5 py-3 text-sm font-medium text-[color:var(--sl-fg-base-soft)] hover:text-[color:var(--sl-fg-base)] hover:bg-gray-50 rounded-lg transition-colors touch-manipulation"
          >
            Cancelar
          </button>
          <button
